...
\____/_/_/ /_/\___/_/ /_/ /_/\__,_/   /____/_/ /_/\__,_/\__,_/\___/_/     /_/    \__,_/\___/_/|_|
        http://en.sbence.hu/        Shader: Prepare side-by-side 3D content for red-blue glasses
*/
...
float4 main(float2 tex : TEXCOORD0) : COLOR {
  ...
  float pixelSize = 1. / width;
	float3 leftPixel = float3(tex2D(s0, tex).r, 0, tex2D(s0, texr).b);
  tex.x += pixelSize;
  texr.x += pixelSize;
  float3 rightPixel = float3(tex2D(s0, tex).r, 0, tex2D(s0, texr).b);
